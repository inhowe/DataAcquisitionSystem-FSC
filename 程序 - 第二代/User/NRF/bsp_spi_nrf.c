...
void SPI_NRF_Init(void)
{
 ...
 /*使能SPI2时钟*/
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_SPI2, ENABLE);
   ...
   /*配置 SPI_NRF_SPI的 SCK-52,MISO-53,MOSI-54引脚 */
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13|GPIO_Pin_14|GPIO_Pin_15;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; //复用功能
  GPIO_Init(GPIOB, &GPIO_InitStructure);  

  /*配置SPI_NRF_SPI的CE,CSN引脚*/
...

{
   ...
   SPI_NRF_WriteReg(NRF_WRITE_REG+RX_PW_P0,RX_PLOAD_WIDTH);//选择通道0的有效数据宽度      
	 ...
	 //SPI_NRF_WriteReg(NRF_WRITE_REG+RF_SETUP,0x01);//设置TX发射参数,18db增益,1Mbps,低噪声增益开启
...
{  
   ...
   SPI_NRF_WriteReg(NRF_WRITE_REG+RF_SETUP,0x0f);  //设置TX发射参数,0db增益,2Mbps,低噪声增益开启   
	 //SPI_NRF_WriteReg(NRF_WRITE_REG+RF_SETUP,0x27);//设置TX发射参数,0db增益,250Kbps,低噪声增益开启  
	 //SPI_NRF_WriteReg(NRF_WRITE_REG+RF_SETUP,0x07);//设置TX发射参数,0db增益,1Mbps,低噪声增益开启 
	 //SPI_NRF_WriteReg(NRF_WRITE_REG+RF_SETUP,0x05);//设置TX发射参数,6db增益,1Mbps,低噪声增益开启
...
u8 NRF_Tx_Dat(u8 *txbuf)
{
   ...
   /*CE为高，txbuf非空，发送数据包 */   
	  ...
	  /*等待发送完成中断 */                            
	while(NRF_Read_IRQ()!=0){	i++; 	if(i==0xFFFF){return ERROR;}} //防止卡死
	/*读取状态寄存器的值 */                              
