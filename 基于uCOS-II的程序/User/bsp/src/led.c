...
void LED_TIM3_Init(void)
{
  ...
  GPIO_Init(GPIOC, &GPIO_InitStructure);
	...
	TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1; //选择定时器模式:TIM脉冲宽度调制模式2
	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable; //比较输出使能
	TIM_OCInitStructure.TIM_Pulse = 999; //设置待装入捕获比较寄存器的脉冲值
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High; //输出极性:TIM输出比较极性高
...
void LED_STATE_Fun(void)
{
	...
	//呼吸
		...
		if(BreathOrFlash_LED1==FASTFLICKER)
		{
			...
			if(cnt1<10)
				TIM3->CCR2=0;
			else if(cnt1>10)TIM3->CCR2=TIM_PULSE_MAX;
			if(cnt1==20)cnt1=0;
		}
		if(BreathOrFlash_LED2==FASTFLICKER)
		{
			...
			if(cnt2<10)
				TIM3->CCR1=0;
			else if(cnt2>10)TIM3->CCR1=TIM_PULSE_MAX;
			if(cnt2==20)cnt2=0;
		}
		//中闪
		if(BreathOrFlash_LED1==MEDIFLICKER)
		{
			...
			if(cnt1<20)
				TIM3->CCR2=0;
			else if(cnt1>20)TIM3->CCR2=TIM_PULSE_MAX;
			if(cnt1==40)cnt1=0;
		}
		if(BreathOrFlash_LED2==MEDIFLICKER)
		{
			...
			if(cnt2<20)
				TIM3->CCR1=0;
			else if(cnt2>20)TIM3->CCR1=TIM_PULSE_MAX;
			if(cnt2==40)cnt2=0;
		}
		//慢闪
		if(BreathOrFlash_LED1==SLOWFLICKER)
		{
			...
			if(cnt1<50)
				TIM3->CCR2=0;
			else if(cnt1>50)TIM3->CCR2=TIM_PULSE_MAX;
			if(cnt1==100)cnt1=0;
		}
		if(BreathOrFlash_LED2==SLOWFLICKER)
		{
			...
			if(cnt2<50)
				TIM3->CCR1=0;
			else if(cnt2>50)TIM3->CCR1=TIM_PULSE_MAX;
			if(cnt2==100)cnt2=0;
		}
}
