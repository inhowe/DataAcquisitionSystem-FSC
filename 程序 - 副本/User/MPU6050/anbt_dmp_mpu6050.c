...
u8 AnBT_DMP_MPU6050_Init(void)	
{
	...
	printf("DMP初始化成功\r\n");
	//
	if(!mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL)) 		;		//完成传感去设定
	else printf("DMP设置传感器错误\r\n");
	//
	if(!mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL)) ;		//完成fifo设置
	else printf("DMP设置FIFO失败\r\n");
	//
	if(!mpu_set_sample_rate(DEFAULT_MPU_HZ))			;			//完成DMP采样率设置
	else printf("DMP设置采样率失败\r\n");
	//
	if(!dmp_load_motion_driver_firmware())				;				//开启DMP功能
	else printf("DMP加载运动固件失败\r\n");
	//
	if(!dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation)))		;		//初始方向设定偏差
	else printf("DMP初始方向设定偏差出错\r\n");
	//
	...
	//
	if(!dmp_set_fifo_rate(DEFAULT_MPU_HZ)) 	;		//DMP  fifo速度
	else printf("DMP设置FIFO速率失败\r\n");
	//
	run_self_test();
	if(!mpu_set_dmp_state(1)) {}				//DMP成功开启
	else printf("DMP启动失败\r\n");				//Dmp启动失败
	//
	return 0;
}
