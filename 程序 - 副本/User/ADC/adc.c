...
void ADC1_Configuration(void)
{
	...
	{	
		...
		ADC_InitStructure.ADC_NbrOfChannel = 10;//规定了顺序进行规则转换的ADC通道的数目
...
void DMA1_Configuration(void)
{
   ...
   DMA_InitStructure.DMA_MemoryBaseAddr = (u32)ADCBuff;//定义DMA内存基地址 放到数组里
   DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;//规定了外设是作为数据传输的目的地还是来源，如果内存作为来源，则内存变量
   DMA_InitStructure.DMA_BufferSize = 10*2;	//通道数*采样量*字节
